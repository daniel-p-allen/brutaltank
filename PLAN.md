...
- **Opt-in trajectory-preview aim assist — implemented** (`client/src/lib/game/render/trajectoryPreview.ts`, `stores/trajectoryHelpStore.ts`, wired into `GameCanvas.svelte`/`FireControls.svelte`). A "Trajectory Help: On/Off" toggle button next to Fire (default off, persisted to localStorage) shows a spread-out dotted preview while aiming, computed from a from-scratch idealized ballistic sim (mirrors `ProjectileSim.java`'s `GRAVITY`/`POWER_SCALE` constants but with every multiplier fixed at 1.0 — no wind, no per-weapon `gravityMultiplier`/`powerScaleMultiplier`, no homing/bounce/tunnel/screen-wrap) — deliberately inaccurate to the real shot, by design, so it stays a rough guide rather than a solved aim.
