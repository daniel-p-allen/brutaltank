...
- **Risk/reward bonus for firing without Trajectory Help — implemented (2026-08-23).** Per explicit user request: "if you choose NOT to use the trajectory help, your earnings are doubled and damage is 25% more." `Fire`'s new `trajectoryHelpUsed` boolean (client-sent, trusted — see shared/protocol.md's own note on why this one field isn't independently re-derivable server-side, same trust level as `angleDeg`/`power`) drives `Match.applyDetonations`'s `NO_HELP_DAMAGE_MULTIPLIER` (1.25x, applied to blast/bounce `centerDamage` before `DamageCalculator.resolve`) and `NO_HELP_CASH_MULTIPLIER` (2.0x, applied to the `CASH_PER_DAMAGE` cash accumulation from that boosted damage) whenever false. The two bonuses **compound** (~2.5x total cash on a no-help shot: 1.25x damage feeding into a 2x cash rate), confirmed as the intended reading with the user rather than an independent flat 2x. Deliberately **not** applied to fall damage (an incidental terrain-collapse side effect, not something the shooter's aim choice caused). Since Trajectory Help is permanently unavailable for Nuke, every Nuke shot always gets this bonus (confirmed with the user, not an oversight — Nuke is a consistent high-risk/high-reward pick as a result). `fireInput.ts`'s `sendFire` computes the flag as `$trajectoryHelpStore && weaponId !== 'nuke'` so a stale toggle-left-on doesn't misreport for Nuke. Regression test: `WeaponAndShieldTest.firingWithoutTrajectoryHelpDoesMoreDamageAndEarnsMoreCash`.
...
- **Bug: a killing shot skips its own flight/explosion animation because the round ends before it plays — filed 2026-08-24, not fixed.** Reported by the user: firing a shot that eliminates the last other player "just ends" the match/round instantly rather than showing the shot's trajectory and explosion first. Root cause (from reading the server flow, not yet confirmed by a client-side trace): `Match.java`'s shot-resolution path broadcasts `RoundEnded` and then calls `openShop()` synchronously, right after `ShotResolved` for the same kill -- all in the same server tick, so the client receives `ShotResolved` -> `RoundEnded` -> `ShopOpened` back-to-back before any client-side flight animation (which is purely client-timed, driven by `shotAnimationStore.ts`/`GameCanvas.svelte`'s `PROJECTILE_ANIMATION_DURATION_MS`) has had time to play. This is the same general category of bug as the two 2026-08-23 fixes above (tank falling before the shot visually lands) -- the client already has a `preShotHeights`/`preShotHealth`/`preShotTankY` pattern for deferring *state* rendering until the flight animation catches up, but nothing yet defers the round-end UI (shop opening, round-end overlay) the same way when the round-ending shot is still mid-flight client-side. Needs scoping: likely a client-side deferral (hold `RoundEnded`/`ShopOpened`'s visible effects until the local flight/impact animation for the killing shot finishes), not a server-side timing change.
- **Bug: MIRV children visually appear to launch from a standstill in all directions instead of continuing the parent's trajectory — filed 2026-08-24, not fixed.** Confirmed the server-side physics is *not* the problem: `Match.java`'s `MIRV` case (~line 1055-1060) already reconstructs each child's launch angle/power from the real apex velocity (`apexSim.finalVx`/`finalVy`) so the underlying simulation is correct. The bug is purely client-side rendering: `projectileRenderer.ts`'s MIRV fall-phase block (~line 200-214) draws each child as a straight-line eased (`fallProgress^2`) interpolation directly from the shared split point to that child's own final impact x/y — a deliberate simplification from an earlier fix (comment cites "children's individual flight paths were deliberately dropped from `ShotResolved.trajectory`... not seeing the falling part of the children"). Since all 4 children's straight-line paths originate at the exact same split point and immediately diverge toward very different landing spots, at low `fallProgress` they visually read as radiating outward from zero velocity rather than continuing an existing arc. Fix would need each child to actually carry (or the client to re-derive) its own post-split trajectory arc, not just a start/end point pair.
...
- **Bug: stale WebSocket connections accumulate in CloseWait, server never reaps them — filed 2026-08-24, not fixed.** Observed live on the deployed VM: `Get-NetTCPConnection -LocalPort 6154` showed 9 connections stuck in `CloseWait` from the nginx proxy machine's IP after normal browser reload/reconnect testing. Root cause: `BrutalTankServer.java`'s `onClose` handler (~line 90-97) only fires when the peer sends a proper WebSocket close *frame*; a browser tab closing/reloading, or a network link dropping (plain TCP FIN/RST, no clean WS handshake), likely never triggers it, so `sessions.remove(...)`/`lobbyManager.handleSocketClosed(...)` never run and Undertow's read side has nothing prompting it to notice the peer closed — the socket just sits in `CloseWait`. Not an artifact of the deploy pipeline debugging that same session; this is an always-there gap in connection lifecycle handling that only became visible once testing exercised a real network path (browser -> nginx -> server) with actual reconnects, instead of local-only same-machine testing. Needs: a ping/pong keepalive (Undertow supports this) plus an idle-timeout that force-closes a channel that's stopped responding, so dead sessions get reaped instead of accumulating indefinitely under real multiplayer usage.
- **Game/round-end UX gaps noted by the user during 2026-08-24 live deploy testing, not yet scoped or built**: (1) the end-of-round/game results screen needs more information shown (exact content not yet specified — revisit with the user); (2) no in-app way to restart/start a new match from the UI once one ends; (3) no in-app instructions/how-to-play for a new player; (4) the user wants to double-check the standard shot's actual trajectory/aim behavior against what's expected (not yet confirmed as a bug vs. a request to verify — revisit).
