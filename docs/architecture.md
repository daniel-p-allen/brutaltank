...
- **New "Weapon Info" page/screen** — a dedicated screen (distinct from the shop and the in-match hotbar) listing every weapon's stats. Per the item below, should also show stock/shop-count; likely a natural home for showing blast radius and other stats already tracked in `WeaponDef` but not currently surfaced anywhere in one place. (The damage rating and weight stars this page was meant to include are already shown on the hotbar itself now — see the damage-pie entry below — so this page's remaining job is stock/owned-qty plus any deeper stats, not a ground-up "add every stat" build.)
