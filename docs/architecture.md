...
- **Bug: Trajectory Help doesn't work for Nuke** (user report, 2026-08-22, investigated but not resolved — confirmed still reproducing on retest after an unrelated ammo-gating fix that was hoped might have incidentally fixed it). Symptom as described: selecting Nuke makes the "Trajectory Help" toggle button visually read as disabled/greyed, and no dotted preview shows, until a different weapon is selected. No code path was found linking weapon selection to that button's styling (it only reacts to `trajectoryHelpStore`'s own on/off state), and manually tracing the preview math for Nuke's multipliers (`gravityMultiplier: 1.25`) doesn't produce an exception or an obviously-empty point list. A defensive `try/catch` was added around the preview compute+draw in `GameCanvas.svelte` (so a crash there, if one is happening, can't silently kill the whole canvas render loop) but the root cause is still unconfirmed. **Next step should be getting a screenshot or screen recording before further investigation** — the verbal description supported two different readings (the aim-preview vs. the actual fired-shot's flight animation) and needed an `AskUserQuestion` round-trip to disambiguate once already.
...
- **TEMPORARY DEBUG-ONLY: manual wind override slider — not yet removed.** Added per explicit user request ("put a temporary 3rd slider in... so i can zero the wind and check everything") to manually verify the wind-direction fix. Adds a `DevSetWind`/`WindOverridden` message pair (`Payloads.java`, `Match.devSetWind`, `BrutalTankServer`'s dispatch case) and an orange-labeled "DEBUG Wind" slider in `FireControls.svelte` (`devInput.ts`). **Every piece is commented "TEMPORARY DEBUG-ONLY" for easy removal** — strip it out (client slider + `devInput.ts`, server message/handler/protocol entry) once wind verification is fully done; it is not meant to ship as a real feature. Not yet documented in `shared/protocol.md` since it's explicitly temporary — if it ends up staying longer than expected, document it properly there instead of leaving it undocumented.
- **Bug: Digger passed straight through a tank without hitting it** (user report, filed not investigated). Reported during live playtest, after the Digger tunnel-penetration-depth fix (70->110, see 469b92c-era commit). Digger's `Behavior.TUNNELING` dispatch keeps integrating through terrain rather than terminating on ground contact — plausible cause: the in-flight direct-tank-hit check (the same `TANK_HITBOX_RADIUS` check every other weapon relies on) may not be getting evaluated once the projectile has entered "tunneling" mode, or Digger's tunnel path passed close enough to register as a graze rather than a hit. Needs reproduction + trace through `ProjectileSim`'s TUNNELING branch and its interaction with the per-step tank-hit check before any fix is attempted.
- **Bug: a tank appears to drop/fall before the projectile has actually landed** (user report, filed not investigated). Reported during live playtest — the tank's fall animation triggered ahead of the incoming shot's impact, rather than in response to it. Not yet root-caused; likely candidates to check first: terrain-collapse/fall-damage logic in `Match.java` (added in the 88a6251 polish pass) firing on a stale/pre-impact terrain state, or a client-side animation-timing mismatch between the projectile's flight animation and the server's actual impact event ordering. Needs reproduction + trace before any fix is attempted.
