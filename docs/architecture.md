...
- **Bug: deploy pipeline reports success but the server isn't actually left running — filed 2026-08-24, not fixed.** See `docs/deployment.md`'s "Known open bug" section for full detail. `Start-Process -WindowStyle Hidden`-launched server process doesn't stay up (confirmed via `Get-NetTCPConnection -LocalPort 6154` coming back empty shortly after a "successful" deploy run), while manually running `.\gradlew.bat run` in a foreground SSH session works and stays running — so the server code itself isn't at fault, something about the detached/hidden process launched from within an SSH-spawned script is getting killed. Needs investigation into Windows job-object/process-lifetime behavior for children of SSH-spawned processes before the automated deploy pipeline can be trusted for real matches.
