...
- **Randomize turn order each round ("roll dice for starting")** — turn order should be determined by a dice-roll/random draw rather than a fixed order, presumably re-rolled each round. Needs scoping: re-rolled every round or just once at match start; does this replace or sit alongside the existing turn-order mechanism (`Match`'s turn queue).
