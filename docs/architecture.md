...
- **Baby Missile shop price increase** — a further bump on top of the 2026-08-25 change (0 → 60, see the earlier "Baby Missile shop price" fix note above); user wants it raised again but gave no target number yet. Needs a specific new price from the user before implementing.
...
- **New expensive "Teleport" shield — lets you move your tank instead of (or in addition to) mitigating damage.** User's own framing: an expensive shield option, exact play mechanics not yet decided ("not sure how we do this yet in terms of play"). This would be a genuinely new category for `ShieldDef` — the current three (Absorb/Deflect/Reflect, see `shared/protocol.md`/`docs/weapon-gap-analysis.md`'s Shield Gap Analysis) all mitigate incoming damage passively once activated; none reposition the tank. Real open questions before scoping: does activating it consume the turn like every other shield activation today (`Fire`'s `weaponId` referencing a shield id, `RESOLVING` sets `activeShieldId` instead of running projectile physics), or is movement a distinct action separate from firing/shielding; how far can it move (fixed distance, player-chosen landing x, random within a range); does it still block/reduce damage like the other shields or is movement its *only* effect; how "expensive" compares numerically to Reflect (currently the priciest shield at 300); and whether relocating a tank has any interaction with terrain collision/fall damage at the new spot (`Match.applyDetonations`'s existing tank-falls-on-terrain-collapse mechanic).
