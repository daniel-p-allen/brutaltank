...
**Completed items (bugs fixed, features shipped, decisions closed) have
been moved out of this list into `PLAN_ARCHIVE.md` (2026-08-25) so this
section stays a list of genuinely open work. Check there first before
assuming something needs re-investigating or re-building.**

- **Bug: bots stay stuck on "Not Ready" after a rematch (`PlayAgain`) — filed 2026-08-25, root cause confirmed, not yet fixed.** User report: played a full match, returned to the lobby via the rematch flow, and the bots' roster badges stayed "Not Ready" (never auto-readying the way they do on initial match creation). Root cause confirmed by reading the code (`server/.../match/Match.java`): `addBot()` explicitly calls `setReady(botId, true)` once, at the moment a bot is first added to a fresh match — but `rematch()` (`Match.java` ~line 352) resets every player's `ready` field to `false`, bots included, and never re-invokes `setReady` for bots afterward (it only resets fields directly on the existing `MatchPlayer` objects, it doesn't call `addBot()` again since the bots already exist). Bots have no client of their own to click Ready in the lobby the way a human does, so once `rematch()` flips them to unready, nothing ever flips them back — the match is stuck waiting on players who can never ready up themselves. Not fixed yet (filed only, per explicit user request) — the fix shape seems straightforward (re-ready every bot player at the end of `rematch()`, mirroring what `addBot()` already does), but leaving that for a dedicated pass rather than a drive-by fix.
...
- **Game/round-end results screen needs more information shown** (exact content not yet specified) — one still-open item from a 2026-08-24 user note whose other two sub-items (a new-match button, an instructions screen) shipped the same day; see `PLAN_ARCHIVE.md`. Revisit with the user on what exactly the results screen should add.
...
- **Standalone "Weapon Info" page for side-by-side comparison — still open, now a smaller ask.** Originally filed as a dedicated screen listing every weapon's stats. The actual informational need it was meant to serve — damage, weight, blast radius, price, owned qty, and (mid-shop) remaining stock, all per weapon — now ships via the hover info card (see below), reachable from wherever the hotbar already is, no new screen/navigation needed. What a standalone page would still add on top of that: seeing every weapon's stats *side by side at once* rather than one at a time via hover. Not scoped or built — open question is simply whether that side-by-side view is still wanted given the hover card covers the original ask.
