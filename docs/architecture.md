...
- **Permanent cross-match leaderboard — requested 2026-08-25, partially scoped 2026-08-25, still not built (plan only, deliberately on hold).** User wants an ongoing running total across games: whoever has the top score for a completed match gets tracked on a persistent leaderboard, not just the single match's final standings. Two of the three open questions are now resolved (user's answers, 2026-08-25):
  - **Identity**: keyed by **display name**, not a real account/login system — simplest option, no new auth flow. Trade-off accepted: no protection against name collisions or someone reusing another player's name.
  - **Score**: **cumulative cash at match end** — already computed, same metric `MatchEnded.finalStandings` ranks by today; no new tracking needed.
  - **Storage is explicitly still open, on hold** — user's own words: "hold off on this for now. We will revisit it." This project currently has zero persistence anywhere (`MatchRegistry`/`Match` live only in server memory, vanish on restart); a leaderboard would be the first real persistent data here, needing a genuine storage decision (flat file vs. embedded DB) before implementation can start. Do not build any part of this — including the two resolved decisions above — until the user explicitly revisits storage.
...
- **Connection/access log for troubleshooting at scale — requested 2026-08-25, revisited 2026-08-25, still not built (plan only, deliberately on hold).** User wants every game logged: every connection/join attempt (successful or failed), with enough detail to troubleshoot problems if a lot of users are playing at once. Explicitly wants it low-resource (plain text, not a database) and date-linked entries (one line/entry per event, timestamped). When re-asked which event set + IP-handling approach to commit to (full event set — WS connect, CreateMatch/JoinMatch success+failure with reason, disconnect, reconnect grace expiry, match end — trusting `X-Forwarded-For` from the known nginx proxy vs. a minimal successful-joins-only log), user's answer was the same as the leaderboard's storage question: **"hold off on this for now. We will revisit it."** So still fully unscoped/unbuilt by design, not by oversight — open questions unchanged: exact event set, what "browser info" means concretely (User-Agent is available from the Undertow handshake; real client IP needs `X-Forwarded-For` read+trusted, not just the proxy's own IP — see `docs/deployment.md`), where the file lives and how it rotates, and any privacy/retention consideration given it would capture player IPs. Do not start implementation until the user explicitly revisits this.

**2026-08-25 batch — filed verbatim from a rapid-fire user list, explicitly "for future consideration," user said they'll discuss further before any of these are built. Not scoped, not built, no code touched:**

- **Randomize turn order each round ("roll dice for starting"), with a graphical dice-roll sequence** — turn order should be determined by a dice-roll/random draw rather than a fixed order, presumably re-rolled each round. User follow-up (2026-08-25): wants this visualized, not just a silent server-side randomization — an actual on-screen dice-roll animation, then a "`<Player>` Starts" banner, a brief pause, then the round begins. Needs scoping: re-rolled every round or just once at match start; does this replace or sit alongside the existing turn-order mechanism (`Match`'s turn queue); exact animation/banner treatment (likely a new client-only sequence gating the existing round-start flow, similar in spirit to the round-end splash — no server protocol change obviously required unless the dice result itself needs to come from the server for fairness/determinism, which it should, being the actual turn-order source of truth).
- **Tanks spawn at random positions, not fixed/evenly-spaced** — needs scoping: fully random x-position per tank, or randomized-but-still-fair (e.g. minimum spacing/no overlap), and whether this re-randomizes every round or only once at match start.
- **Show each weapon's damage stat to the player** — surfaced twice in the same list ("Damage on the weapons" and "You need the damage it does"), also called out as part of the Weapon Info page below. Needs scoping: shown where (hotbar tooltip, shop card, dedicated info page, all three), and whether it's a single number (`centerDamage`) or also conveys blast radius/falloff.
- **Health meter** — user's own wording, no further detail given yet. Current HP is shown as a number in each player card (`.player-card`); unclear whether this asks for a visual bar/meter instead of/alongside the number, or something else entirely. Needs the user's follow-up before scoping.
- **No way to exit to a new game from mid-match or from the lobby — need a "new game" button in the lobby.** User's own framing: neither being in an active match nor being in the lobby currently offers a way out to start a fresh game. Note: `PostMatchScreen`'s "Back to Start"/`PlayAgain` (added 2026-08-24) already covers the *post-match* case — this request is about the lobby and in-match states specifically, which have no equivalent exit today. Needs scoping: does "new game" mean leave this match/lobby and return to the menu to create/join a different one, or reset the current lobby in place (similar to what `PlayAgain` already does post-match)?
- **Track total damage dealt across all three rounds of a match, not just per-round** — a cumulative match-wide damage stat, presumably shown somewhere in the UI (player cards, end-of-match standings, or the Weapon Info page). Needs scoping: is this per-player total damage dealt, damage taken, or both; where it's displayed; whether "three rounds" is describing the current default round count or is itself a fixed assumption to double check.
- **New "Weapon Info" page/screen** — a dedicated screen (distinct from the shop and the in-match hotbar) listing every weapon's stats. Per the two items below, at minimum should show damage and stock/shop-count; likely a natural home for showing weight-class stars, blast radius, and other stats already tracked in `WeaponDef` but not currently surfaced anywhere in one place.
- **On that page, show both the match's remaining shop stock and the player's own current owned quantity, per weapon** — two distinct numbers per weapon: the shared `shopStock`-derived stock remaining (already broadcast via `ShopUpdate.stockRemaining`) and the player's own current ammo count (already tracked per-player, shown today only on the in-match hotbar buttons as `×N`). Needs scoping: whether this new page is shop-phase-only or also viewable mid-match/mid-turn.
- **Baby Missile shop price increased again, 60 → 200 — done 2026-08-25.** Explicit user-specified number (following the earlier 0→60 fix noted above). Now mid-pack (tied with Tunneling Shot at 200, above Digger's 120 and Heavy Cannonball's 150, below Bouncing Betty/Napalm/Cluster Bomb/MIRV/Nuke), reflecting its homing/terminal-guidance advantage rather than just its lightest/weakest-blast stats. `WeaponDef.BABY_MISSILE`, server tests re-run green.
- **Trajectory Help should default off each turn and, once turned on for a turn, can't be turned back off that same turn.** Currently `trajectoryHelpStore`'s on/off state is a simple persistent toggle the player can flip freely at any time (see the 2026-08-23/24 Trajectory Help sections above) — this asks for two behavior changes: (1) it resets to off at the start of every turn rather than remembering the last state, and (2) within a turn, once enabled it becomes one-way (can't un-enable to reclaim the "skip it for +25% cash" bonus after seeing the preview). Needs scoping: does "off each time" mean every turn, or only every round; interacts directly with the existing `NO_HELP_CASH_MULTIPLIER` reward logic (`Fire.trajectoryHelpUsed`), since a one-way toggle closes a possible exploit where a player could preview the shot before committing to the no-help cash bonus.
- **New expensive "Teleport" shield — lets you move your tank instead of (or in addition to) mitigating damage.** This would be a genuinely new category for `ShieldDef` — the current three (Absorb/Deflect/Reflect, see `shared/protocol.md`/`docs/weapon-gap-analysis.md`'s Shield Gap Analysis) all mitigate incoming damage passively once activated; none reposition the tank. User's follow-up mechanic (2026-08-25): when hit while active, it halves incoming damage (like Absorb's existing -50% mitigation) — and *if the tank survives that hit*, it's then teleported to a random location on the map. So it's damage mitigation + a conditional-on-survival relocation, not movement as its own standalone action. Real open questions still before scoping: does activating it consume the turn like every other shield activation today (`Fire`'s `weaponId` referencing a shield id, `RESOLVING` sets `activeShieldId` instead of running projectile physics); does it only trigger relocation on the *first* hit while active or every hit (single-use like Deflect, or reusable like Absorb/Reflect until the round resets); how "random location" avoids landing inside/under existing terrain or another tank; how "expensive" compares numerically to Reflect (currently the priciest shield at 300); and whether relocating a tank has any interaction with terrain collision/fall damage at the new spot (`Match.applyDetonations`'s existing tank-falls-on-terrain-collapse mechanic).
