...
- **Show each weapon's damage stat to the player, as a graphical rating (like the existing weight-class stars), not just a raw number** — surfaced twice in the same list ("Damage on the weapons" and "You need the damage it does"), also called out as part of the Weapon Info page below. User follow-up (2026-08-25): wants a visual damage rating mirroring the existing 3-tier ★ weight-class system (`weaponSelectStore.WEAPON_CATALOG`) rather than (or alongside) a plain number. Needs scoping: shown where (hotbar tooltip, shop card, dedicated info page, all three); a single number (`centerDamage`) vs. also conveying blast radius/falloff; and how a damage rating (presumably tiered by `centerDamage` value, similar to how weight is tiered by `gravityMultiplier`) avoids visually clashing with the existing weight stars on the same weapon button.
