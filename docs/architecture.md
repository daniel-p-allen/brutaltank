...
| Nuke | **shipped** — white-hot flash → smoke puffs, bigger radius and longer duration than every other weapon (`NUKE_SMOKE_PUFF_COUNT`/`NUKE_FLASH_DURATION_MS`/`NUKE_EFFECT_DURATION_MS` in `projectileRenderer.ts`) | white-hot core → orange → smoke, unchanged |
